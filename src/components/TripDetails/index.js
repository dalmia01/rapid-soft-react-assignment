...
const TripDetails = (props) => {
    ...
    /** filter trip data based on valid start and end dates */
    let filteredTripDetails = [...tripDetails.tripDetails];
    filteredTripDetails = tripDetails.tripDetails.filter((trip) => {
        if (
            calenderStartDate !== "" &&
            calenderEndDate !== "" &&
            new Date(calenderStartDate) <= new Date(parseInt(trip.startDay)) &&
            new Date(calenderEndDate) >= new Date(parseInt(trip.startDay))
        ) {
            return trip;
        }
    });
    /** setting trip to initial trip detaisl if dates are invalid*/
                        ...
                        <div className="trip-header">
                            <div className="trip-item-details-complete">
                                ...
                                <div className="distance-details">
                                    <span className="blue-highlight">Total KM: {totalKms} KM</span>{" "}
                                    <span className="yellow-highlight">Total Expense: {amount}</span>
                                </div>
                                <div className="collapsable" onClick={collapseHandler}>
                                    -
                                </div>
                            </div>
                                                ...
                                                <tr key={"tripList" + index}>
                                                    ...
                                                    <td>
                                                        {item.tripExpenses.length > 0 ? (
                                                            <span className="tripe-expense">
                                                                Rs{" "}
                                                                {item.tripExpenses.reduce((acc, curr) => {
                                                            ...
                                                            <span className="tripe-expense">
                                                                Rs 0 <i className="fa fa-info-circle"></i>
                                                            </span>
                                                        )}
                                                    </td>
                                                    ...
                                                    <td>
                                                        <Button
                                                            btnText="Movement Report"
                                                            color="#01c5c4"
                                                            btnClickHandler={btnClickHandler}
                                                        />
                                                        <Button
                                                            btnText="Stoppage Report"
                                                            color="#01c5c4"
                                                            btnClickHandler={btnClickHandler}
                                                        />
                                                    </td>
